...
- **Asking the other session** → `host/tools/notes.py` — an append-only file (`/tmp/applebridge_notes.log`), because two sessions here share a filesystem and see a file at once, where an MCP tool stays invisible until the other side's server restarts. `notes.py ask "…"` deposits a question, `notes.py answer <ts> "…"` closes it, and the brief prints what is still open **and the answers to what this session asked**. **Set `APPLEBRIDGE_WHO` per session** — it is a precondition, not cosmetics: with the default both sides are called `agent`, so an answer cannot be addressed back to the asker and the return path routes nothing (the brief says so rather than staying quiet). **The limit is not the channel:** a session has no event loop, so an answer arrives at the other side's *next turn* — for an answer needed now, the human is still the fastest path (built 2026-08-02).
