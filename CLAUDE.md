...
- **Asking the other session** → `host/tools/notes.py` — an append-only file (`/tmp/applebridge_notes.log`), because two sessions here share a filesystem and see a file at once, where an MCP tool stays invisible until the other side's server restarts. `notes.py ask "…"` deposits a question, `notes.py answer <ts> "…"` closes it, and the brief prints whatever is still open. Set `APPLEBRIDGE_WHO` per session so the file reads. **The limit is not the channel:** a session has no event loop, so an answer arrives at the other side's *next turn* — for an answer needed now, the human is still the fastest path (built 2026-08-02).
