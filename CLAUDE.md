...
- **Asking the other session** → `host/tools/notes.py` — an append-only file (`/tmp/applebridge_notes.log`), because two sessions here share a filesystem and see a file at once, where an MCP tool stays invisible until the other side's server restarts. `notes.py ask "…"` deposits a question, `notes.py answer <ts> "…"` closes it, `notes.py note "…"` states something nobody has to answer — use it for a status report, or it registers as a question that stays open forever. The brief prints what is still open **and whatever is addressed to this session**. Sessions name themselves from `CLAUDE_CODE_SESSION_ID` (`sess-9e5cc132`), so nothing needs configuring; `APPLEBRIDGE_WHO` overrides it when a name says more than an id. If neither is in the environment the brief reports that nothing can be addressed, rather than routing silently into the void. **The limit is not the channel:** a session has no event loop, so an answer arrives at the other side's *next turn* — for an answer needed now, the human is still the fastest path (built 2026-08-02).
