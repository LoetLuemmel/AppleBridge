...
## Start the stack
**AppleBridge ships on `ether slirp` (D-019).** That is the configured, documented, measured and developed-on branch; `etherhelper` stays supported and hand-configured but is not maintained toward parity. The cost is a capability we have and do not deliver by default: `NBPLOOK`, `AFPMOUNT` and `mac_appletalk_browse` are built and tested, and **not exercisable in the shipping configuration**.

**On a machine that has never run it:** `cd host && ./install_bridge.py --dry-run`, then without the flag. It configures the **slirp** branch and nothing else (D-018): it discovers the emulator bundle, writes `ether slirp`, generates `host/local.env` with **no** host address (slirp needs the `0.0.0.0` bind), installs the launchd agent, and prints the guest-side values — labelled by *whose* address each one is, which is the R5 trap. The guest's TCP/IP step is now **one dropdown**: slirp answers DHCP and supplies all four values including the name server (measured 2026-07-28), so the field that gets left empty cannot be. It **refuses** a host already running `etherhelper` rather than converting it (`--force-slirp` overrides), and refuses to touch prefs while an emulator is running. `--seed-guest-prefs <image.dmg>` writes `IP=` into a powered-off image (bytes + CR endings, every other key preserved). **No AppleTalk on that branch** — that is the stated cost, not a fault, and `bridge_doctor` reports a configured slirp host as INFO for the same reason.

**One-shot (a configured machine):** `cd host && ./start_stack.sh` — on the `etherhelper` branch it places the host alias on the default-route interface (the freeze-avoidance rule below) through one admin prompt; on **slirp** it skips the privileged block entirely and asks for nothing. Then it (re)starts the host server and launches the emulator named by `APPLEBRIDGE_EMULATOR_APP`. Then do the two in-emulator steps (daemon + ToolServer) it prints. Manual steps:
...
## Where things stand
AppleBridge runs as a **System 7 background service** with an optional on-screen monitor. The daemon speaks **wire protocol v0.2** (version negotiation + optional mutual auth); the host server auto-starts via launchd. Current daemon **0.8d32** (`mac/vers.r` is the single source for that number); the MCP surface is **30 tools** (`len(TOOLS)` in `mcp/tools.py`). Validated live on **both** Basilisk II (System 7.6.1) and SheepShaver (PowerPC / Mac OS 9), and on real hardware — a Macintosh **SE/30** over RS-422.
...
- **Operating notes** → **`docs/OPERATING_NOTES.md`** (D-020). What an agent must *verify* before believing the bridge, as opposed to what it must *know*: the two line endings inside one response, silent success as the default failure, checking a build by its artefact rather than its status, where MPW Shell's "missing" output actually is. Read it when a command reports success and nothing happened — and **grep it**, which is the reason it is a file: it lived on a CMS page behind Authelia until 2026-08-03, where the one moment it was needed (mid-debug) was the one moment it could not be searched. It is appended to, not rewritten — add what cost you a session (started 2026-08-02, after the same parse trap was rediscovered three times in one sitting).
- **Asking the other session** → `host/tools/notes.py` — an append-only file (**`~/.applebridge/notes.log`** — it lived in `/tmp` until a restart on 2026-08-05 took 130 notes with it, silently, past every guard the tool has; a legacy `/tmp` channel is carried over once, and only for the default path), because two sessions here share a filesystem and see a file at once, where an MCP tool stays invisible until the other side's server restarts. `notes.py ask "…"` deposits a question, `notes.py answer <ts> "…"` closes it, `notes.py note "…"` states something nobody has to answer — use it for a status report, or it registers as a question that stays open forever. `list` shows only what is OUTSTANDING (open questions + what arrived since you last wrote) — `--all`/`--full` for the rest, after the default view printed 15 000 characters of already-handled material on a day with zero open questions. **Write anything long or technical with `--stdin` and a quoted heredoc delimiter** (`notes.py note --stdin <<'EOF'`): on 2026-08-04 both sides silently lost text to the shell — `$0A1C` became an empty variable, a backquoted `nc` was *executed* and deleted two sentence subjects — and the channel reported zero unreadable lines, correctly, because the line it received was perfect. An unquoted `<<EOF` expands the same metacharacters, so the quotes are the mechanism. `notes.py rotate` archives the file and starts a fresh one (refused while a question is open — an archived question can never be closed), and `notes.py find <text>` searches the live channel **and** the archives, because an archive nobody can search is a bin with a better name. The brief prints what is still open **and whatever is addressed to this session**. Sessions name themselves from `CLAUDE_CODE_SESSION_ID` (`sess-9e5cc132`), so nothing needs configuring; `APPLEBRIDGE_WHO` overrides it when a name says more than an id. If neither is in the environment the brief reports that nothing can be addressed, rather than routing silently into the void. **The limit is not the channel:** a session has no event loop, so an answer arrives at the other side's *next turn* — for an answer needed now, the human is still the fastest path (built 2026-08-02).
- **Session bracket**: a SessionStart hook prints `host/tools/session_brief.py` (version, branch, decisions, open questions, open ledger items — state, not history); the Stop hook runs `ledger_diff.py --quiet` (silent when in step).
