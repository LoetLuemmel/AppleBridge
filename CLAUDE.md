...
- **Operating notes** → [`pit.390er.de/applebridge/workmode/agent-operating-notes-verification-and-traps/`](https://pit.390er.de/applebridge/workmode/agent-operating-notes-verification-and-traps/) (behind Authelia). What an agent must *verify* before believing the bridge, as opposed to what it must *know*: the two line endings inside one response, silent success as the default failure, checking a build by its artefact rather than its status, where MPW Shell's "missing" output actually is. Read it when a command reports success and nothing happened. It is appended to, not rewritten — add what cost you a session (started 2026-08-02, after the same parse trap was rediscovered three times in one sitting).
