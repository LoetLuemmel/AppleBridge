...
## D-023 — on a Linux host there is one backend, because the other does not exist there

- **Date:** 2026-08-18
- **Status:** superseded → D-024 (2026-08-18, same day — the falsifier below fired within hours)
- **Decision:** where AppleBridge runs on a **Linux host**, the emulator's Ethernet backend is **`ether slirp`**, and there is no second branch to choose, fall back to, or hand-configure. The tooling therefore does not probe for a backend on Linux, does not offer one, and states the AppleTalk cost as a property of the platform rather than as a configuration away. This is **not** D-018/D-019 repeated for another operating system: those set `etherhelper` aside on a host where it exists and works. Here it does not exist. What follows is what a port has to build (a platform seam for discovery, service supervision and address probes) and what it must **not** build (a backend choice).
- **Evidence:** `etherhelpertool` is a macOS binary shipped inside the macOS `.app` — the installer looks for it at `Contents/Resources/etherhelpertool`, a path with no Linux counterpart, and D-018's own correction established that its provenance is the kanjitalk755 macemu fork's macOS build. Linux emulators reach the LAN through TUN/TAP or the `sheep_net` kernel module, and **R9** (`docs/INSTALLER_REQUIREMENTS.md`) already forbids requiring a kernel extension, so that path is closed by a rule this project wrote before Linux was in view. Everything the project learned the hard way about host networking is a property of the absent backend: the host address on the default-route interface (`TROUBLESHOOTING.md`), the MACNAT return path, the single-NIC hairpin (D-015), the bridge (D-016, D-017), the two interactive password prompts (D-018). What remains is slirp, whose addressing is **internal to the emulator** — `10.0.2.15` / router `10.0.2.2` / resolver `10.0.2.3`, answered by slirp's own DHCP, server bound to `0.0.0.0`, and `10.0.2.2` forwarded to the host's loopback (measured on two hosts and two guests, 2026-07-28 and 2026-07-31). Occasion: a reader running SheepShaver on Linux patched `install_bridge.py`'s macOS paths himself and reported it, 2026-08-17 ([emaculation t=12754](https://www.emaculation.com/forum/viewtopic.php?t=12754)); assessment in [*AppleBridge on a Linux Host*](https://td5.390er.de/applebridge/applebridge-on-a-linux-host-slirp-only-assessment/).
- **What this entry does not claim:** that Linux works. **Nothing has been executed on a Linux host, and this project has none.** It decides the *shape* of Linux support so the work is not spent on a backend choice that has no second option — the open item is the ledger's, and the machine that will decide whether the seam is right belongs to the reporter.
- **Cost, stated rather than buried:** on Linux there is **no AppleTalk at all**, and the wording matters. On macOS (D-019) the cost is a capability we have and do not *deliver by default* — a user who wants the Chooser and AFP mounts configures `etherhelper` by hand. On Linux that sentence has no second half: `NBPLOOK`, `AFPMOUNT`/`AFPUNMOUNT` and `mac_appletalk_browse` are **not reachable on that platform**, by anyone, in any configuration this project supports. A second limit will be met at the same time and is *not* implied by this entry: the host-side real-mouse layer is `cliclick`/`osascript`/`screencapture`, so until an X11 equivalent exists a Linux host cannot drive menus, Standard File lists or modal dialogs — that is a separate piece of work, and conflating the two would let a missing input layer be excused as a network decision.
- **Revisit if:** an emulator build on Linux offers a bridged backend that needs **neither a kernel module nor a privilege prompt** — then Linux has a second branch and this entry is short a case, exactly as D-001 was when it silently assumed two interfaces. Equally if the slirp guest values above turn out **not** to hold on a Linux host: they are measured on macOS, and the load-bearing leg is that the addressing is emulator-internal. If that leg fails, this decision must be re-derived rather than re-stated.

## D-024 — on Linux the installer ships slirp, and the other backends are hand-configured

- **Date:** 2026-08-18
- **Status:** active
- **Supersedes:** D-023, on the same day. That entry said *"on a Linux host there is one backend, because the other does not exist there"* and one of its two legs was false. It is kept rather than edited, because a decision that was wrong for six hours is evidence about how this project reasons, and because its wording had already been published.
- **Decision:** on a **Linux host** the installer configures **`ether slirp` and nothing else** — the same scoping D-018 applies on macOS, and for the same reason: it is the only backend that installs, starts and reconnects with no privileged step and nobody at the keyboard. Linux is **not** a one-backend platform. `sheep_net`, `tap`/`tun` and `vde` all exist in the Unix build, **AppleTalk is reachable on Linux**, and those paths are supported the way `etherhelper` is supported on macOS: documented, hand-configured, not installed for you. A Linux port must therefore keep the backend a *value it reads and can write*, not a constant it assumes.
- **Evidence:** source, not inference (`kanjitalk755/macemu` at `f06fa0e`, `BasiliskII/src/Unix/ether_unix.cpp`, read 2026-08-18): the Unix build selects `tap*` → ethertap, `tun` → tuntap, `slirp`, `vde` / `vde:<sock>`, and **anything else → sheep_net**; `etherhelper` is compiled in only under `#ifdef ENABLE_MACOSX_ETHERHELPER`. So D-023's first leg holds and is now checked in source rather than assumed: **the macOS helper genuinely does not exist on Linux.** Its second leg does not hold. Only **sheep_net** needs a kernel module — out-of-tree, and rebuilt after every kernel upgrade. **`tun` needs no module**: it runs the `tunconfig` script, which requires `sudo` **NOPASSWD** entries for `ifconfig` and `iptables` and refuses to run without them — a one-time sudoers change, not a per-launch password prompt — and it sets up **IP masquerading**, i.e. a routed path that carries only what Linux itself routes. **`vde` needs neither module nor root** on the guest side. AppleTalk therefore reaches a Linux guest two ways: directly over `sheep_net`, or **routed** through netatalk over ethertap — and this project's LAN already runs netatalk 2.2.2. Survey: [*Does the Linux Build of Basilisk II Support AppleTalk?*](https://td5.390er.de/applebridge/does-the-linux-build-of-basilisk-ii-support-appletalk/).
- **What this corrects, and where it was published:** D-023 read "the Linux bridged path needs a kernel module, which R9 forbids requiring" — true of `sheep_net`, false of `tap`/`tun`/`vde`, and stated as though it covered all of them. R9 still stands and still rules out **requiring** sheep_net; it never ruled out a path that needs no module. The same wrong sentence went into the public assessment [*AppleBridge on a Linux Host*](https://td5.390er.de/applebridge/applebridge-on-a-linux-host-slirp-only-assessment/) and into a drafted forum reply; the article carries a dated correction and the reply was fixed before posting. **The cost sentence was the damaging part**, because it was the quotable one: "on Linux AppleTalk is not reachable by anyone in any supported configuration" is simply untrue. The truthful version is the boring one — *not installed by default, available by hand* — which is what macOS has said all along.
- **Why the decision survives its own evidence:** nothing above makes another backend *installable unattended*. sheep_net wants a module built against the running kernel; tun wants sudoers edited first and gives a routed IP path rather than a bridge; vde wants a switch process and, to reach the physical LAN, a privileged plug. D-018's criterion — an installer whose output cannot start without a human at the keyboard has not finished its job — selects slirp on Linux exactly as it does on macOS. What changes is the *reason*, and reasons are what this file is for.
- **Cost, stated rather than buried:** the default Linux path has no AppleTalk, the same cost D-019 states for macOS and no worse. The new cost is documentary: three hand-configured branches to describe instead of one, on a platform this project cannot test.
- **Revisit if:** `vde` is measured to carry AppleTalk between a guest and a netatalk host **without root** — that would make an unattended AppleTalk install possible on Linux and on no other platform, which is an installer branch rather than a footnote. Equally if `sheep_net` stops building against current kernels (then that branch documents a dead path and should say so), or if a Linux user turns out to want AppleTalk immediately, which is the same falsifier D-018 carried and the one that has now fired twice on the neighbouring question.

## D-025 — screenshot compression lives in the guest, not in the emulator or the host

- **Date:** 2026-08-30
- **Status:** active
- **Decision:** the cost of *seeing* the guest is reduced where the pixels are: the daemon crops to the requested rectangle, packs rows with the Toolbox `PackBits`, and sends an XOR row delta against the frame it retains (`SCREENSHOT2`, daemon 0.8d46, `docs/SCREENSHOT_V2.md`). Neither a VNC/RFB server nor an emulator patch exporting the framebuffer is pursued. A full RFB server in the daemon was assessed and declined; its two useful ideas — an incremental update of a requested rectangle, and a native 8-bit colour-map pixel format — are adopted into the existing verb.
- **Evidence:** measured 2026-08-30 on Basilisk II / System 7.6.1 / `ether slirp`. Before: 4.4 s and 787 KB per look, of which the base64 leg everyone suspected was milliseconds and the host PNG encode 0.31 s; time-to-first-byte on `:9001` equalled the total, i.e. the raw 768 KB pixmap crossing a 68K OT stack at ~200 KB/s was the whole cost. After: 2.7 s / 578 KB for the first full frame on a link (PackBits packs a dithered desktop only 1.33:1), **0.14 s / 2.8 KB** for every following look, 0.45 s / 85 KB for a 400×300 region; the host encode fell to 4 ms with an indexed PNG. A delta-composited frame matched an independent guest crop in 120 000 of 120 000 pixels.
- **Why not the alternatives:** a guest-side RFB server would be a second protocol state machine in cooperative 68K code serving a consumer — an LLM — that wants one picture per step, not a stream, and its input path adds nothing over `KEY`/`CLICK`; reachability would be solved (reverse RFB to a listening viewer, or slirp's `redir tcp:5900::5900`, which the installed BasiliskII binary carries and nobody has used), but nothing else would. A host-side capture of the emulator window (`screencapture -l`, 0.18 s measured) or a Basilisk patch exporting `the_buffer` would be faster still and help exactly one of the three targets: not SheepShaver as shipped, not the SE/30 over RS-422, whose 57 600 baud is where 768 KB hurts most. macOS Screen Sharing already gives a *human* a live view with the real mouse at zero code; that is the VNC this project needs, and it is not the agent's.
- **Cost:** one retained screen in the daemon's partition (768 KB at 1024×768×8), a second wire format to keep in step on both sides (the legacy verb stays for old hosts), and the first look on each link still pays a 2.7 s full frame.
- **Measured after the decision (2026-08-30, same day):** the encoder question was put to the framebuffer itself in THINK C (`docs/SCREENSHOT_V2.md`, *Measured encoders*). PackBits packs the desktop 1.39:1, an Up predictor in front of it 13.6:1 at +21 ms of 68K time; LZSS 8:1 at 1.2 s. enc 3 shipped in 0.8d47 and the first look fell from 2.7 s to 0.45 s. The row scan costs 7.5 ms, so a dirty-rect hook is a no-go. The host-side export was built as a spike and measured at 12–31 ms per frame — the decision stands because that number helps one target of three, not because it is small.
- **Revisit if:** a target appears where guest CPU rather than the link dominates (PackBits + XOR on a real 68030 at 512×342×1 is a few KB and was not the bottleneck; a 32-bit 1024×768 screen on a slow CPU might invert that), or if a consumer wants a *stream* — a human watching the agent work — in which case RFB's framing is the right one and the delta machinery here is its update encoder.

## D-026 — the guest speaks TLS itself: Retro68 + Crypto Ancienne, not a host gateway

- **Date:** 2026-08-30
- **Status:** active
- **Decision:** software that needs modern cryptography on the 68K guest is built on the host with Retro68 (m68k gcc, Multiversal interfaces) against Crypto Ancienne, and deployed over the bridge; the socket layer is the MacTCP driver API, which Open Transport serves through its compatibility layer. The host-side TLS gateway (`examples/webpeek/webpeek_gateway.py`) stays as the route for period browsers such as Navigator 3, which cannot be relinked; it is no longer the route for software this project writes. Delivered on that basis: `httpsget` behind `mac_https_get`, and WebPeek 2 — a reader that resolves names, negotiates TLS 1.3 and verifies the chain on the Macintosh with nothing running on the host.
- **Evidence:** MPW `SC` has no 64-bit integer (`MeinMac:MPW:WebPeek:lltest.c`, 2026-08-28: `ULL` is a lexical error), and the library needs one in 108 places outside its bignum core; rewriting that is not a spike. Retro68 built natively in ~50 min; cryanc compiled for 68K at the first attempt behind two include shims. Measured on Basilisk II / 68030 model / System 7.6.1: TLS 1.3 handshake 0.28 s (X25519, local server), 1.6 s (P-384, public server), 3.2 s through a HelloRetryRequest to P-521, 5.5 s with the certificate chain verified; a 27 KB page fetched and verified in 6.3 s wall clock; a plain-http page in 1.5 s. Two library defects found and submitted upstream on the way: no client-side HelloRetryRequest, and a TLS 1.3 certificate parser that verified nothing (off-by-one on the context byte) — the second found only by a negative test, which is now the standard for any verifier here.
- **Why not the alternatives:** keeping TLS on the host makes the guest a thin client of a Python process that must be running — it fetches nothing on its own, and the SE/30 over RS-422 would be as dependent as the emulator. Porting a smaller library (a minimal TLS 1.3 in C89) is a project in itself and would still need a 64-bit integer that `SC` lacks. Patching Netscape's socket layer is not on any table.
- **Cost:** a second toolchain on the host (2 GB of source, a 50-minute build, its own trap list: no `CONSOLE` library on this guest, `-fno-jump-tables` with gcc 16, `<strings.h>` colliding with the Toolbox header on a case-insensitive disk); code built this way cannot be linked with `SC` objects, so a program is either MPW or Retro68, never both; a 4–6 s handshake on the emulator and an unmeasured multiple of that on real hardware; a 13-root bundle that will age.
- **Revisit if:** real hardware measures handshakes that a person will not wait for (cryanc's own figure is 22 s for TLS 1.2 on a IIci; the SE/30 has not been measured), or if the two upstream patches are declined and a private fork has to be carried, or if a target appears where MacTCP's API is absent and only Open Transport exists — then the socket layer, not the decision, is what moves.
