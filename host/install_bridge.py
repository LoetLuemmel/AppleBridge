...
def guest_prefs_text(host_ip, net="OT"):
    ...
    if not host_ip:
        # RELEASE kit: no address at all. A published artifact cannot carry the
        # building machine's address — on any LAN where that number answers, a
        # stranger's daemon connects to a stranger's computer and reports full
        # health (R2), and a release multiplies that by everyone who downloads
        # it. Empty is the only honest value, and the daemon already refuses to
        # dial without one and says why.
