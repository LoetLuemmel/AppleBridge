...
def guest_prefs_text(host_ip, net="OT"):
    ...
    if not host_ip:
        # RELEASE kit: no address OF THIS MACHINE. A published artifact cannot
        # carry the building machine's address — on any LAN where that number
        # answers, a stranger's daemon connects to a stranger's computer and
        # reports full health (R2), and a release multiplies that by everyone
        # who downloads it.
        #
        # What it ships instead is GUEST_ROUTER, the slirp constant, which names
        # no machine: it means "whichever host runs this emulator". This comment
        # said "empty is the only honest value" for a month after the code had
        # stopped doing that (corrected 2026-08-01) — the same stale claim the
        # README carried, and the reason `test_the_readme_states_the_address_a
        # _released_kit_actually_ships` now derives it from here instead.
        return ("# AppleBridge preferences - generated by install_bridge.py\n"
                ...
                "#\n"
                "# IP= is THE HOST's address - the machine this guest should dial,\n"
                "# not the guest's own.\n"
                "#\n"
                "# The address below is not a guess and not the address of the\n"
                "# machine that built this kit. Under slirp it is a constant of the\n"
                "# branch, the same on every machine, and it reaches whichever host\n"
                "# runs the emulator: slirp forwards it to that host's loopback, and\n"
                "# the server hears it there because it binds every address\n"
                "# (measured 2026-07-31 on two hosts and two guests).\n"
                "#\n"
                "# Change it if the bridge server runs on a DIFFERENT machine than\n"
                "# the emulator: then the loopback is the wrong host, and only that\n"
                "# machine's own address can say so. AppleBridgeConfig on the guest\n"
                "# edits this, or `install_bridge.py --seed-guest-prefs <kit.dmg>`\n"
                "# writes it before you mount the kit.\n"
                f"IP={GUEST_ROUTER}\n"
                f"NET={net}\n")
    return ("# AppleBridge preferences - generated by install_bridge.py\n"
            "# HOME= is set by the guest installer, which knows where it put things.\n"
...
def export_guest_kit(dest, host_ip, probes, run=None, exists=None,
    ...
    try:
        ...
        prefs_text = guest_prefs_text("" if release else host_ip)

        # A RELEASE kit may carry exactly one address: the slirp constant, which
        # names whichever machine runs the emulator and therefore cannot point
        # anywhere in particular. Any OTHER address is this builder's, and
        # publishing it would hand every downloader a guest that dials THIS host
        # and reports full health — R2, multiplied by the user base. The
        # binaries are covered by the literal scan above; the prefs file is the
        # one place an address legitimately lives, so it is checked separately.
        #
        # The check used to demand IP= be EMPTY. That was right while the guest
        # had no way to reach a host it had not been told about; once 10.0.2.2
        # was measured to work (2026-07-31, two hosts), an empty field stopped
        # being caution and became a question asked for no reason.
        if release:
            found = [a for a in payload_host_literals(prefs_text.encode("mac_roman"))
                     if a != GUEST_ROUTER]
            if found:
                return (False, "refusing to publish a release kit whose prefs "
                               "carry a specific host address: " + ", ".join(found)
                               + f". A released kit may ship only {GUEST_ROUTER}, "
                                 "which means whichever machine runs the "
                                 "emulator; anything else names this one.", [])

        write_bytes(prefs_blob, macbinary.encode(
            prefs_text.encode("mac_roman"),
    ...
    msg = (f"kit image with {len(placed)} items: {image_path}. "
           ...
           + (f"Its prefs carry IP={GUEST_ROUTER} — not this machine's address, "
              "which a published kit must never carry, but the slirp constant "
              "that means whichever host runs the emulator. So it installs and "
              "connects with no address question at all. Change it only if the "
              "bridge server runs on a DIFFERENT machine than the emulator: "
              f"`--seed-guest-prefs {image_path}` before mounting, or "
              "AppleBridgeConfig on the guest afterwards."
              if release else f"Prefs already carry IP={host_ip}."))
    if missing:
...
def build_parser():
    ...
    p.add_argument("--release-kit", action="store_true",
                   help="build a kit for PUBLICATION: identical to "
                        "--export-guest-kit except its prefs carry no host "
                        "address at all. Required for anything you hand to "
                        "someone else — a kit carrying this machine's address "
                        "points a stranger's guest at this computer, and it "
                        "connects and reports full health. The downloader "
                        "supplies the address with --seed-guest-prefs or in "
                        "AppleBridgeConfig.")
    ...
    if kit_dir is not None:
        ...
        if not dest:
            kit = (False, "no disk image in the emulator prefs to place the "
                   ...
                   [])
        elif not host_ip and not args.release_kit:
            ...
            kit = (True, f"dry run: would build the kit image {target} "
                         f"(volume `{KIT_VOLUME}`) with "
                         + ("NO address (release kit)" if args.release_kit
                            else f"IP={host_ip}")
                         + f" ({names} + prefs)", [])
        else:
