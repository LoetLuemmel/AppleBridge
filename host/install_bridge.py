...
def guest_prefs_text(host_ip, net="OT"):
    ...
    if not host_ip:
        ...
        return ("# AppleBridge preferences - generated by install_bridge.py\n"
                ...
                "# not the guest's own. It is deliberately EMPTY in a released kit:\n"
                "# only you know it. Set it with `install_bridge.py\n"
                "# --seed-guest-prefs <this kit.dmg>` before mounting the kit, or in\n"
...
def export_guest_kit(dest, host_ip, probes, run=None, exists=None,
    ...
    try:
        ...
        # A RELEASE kit must carry NO address anywhere — the binaries are
        # already guarded by the literal scan above, and this is the one file
        # where an address legitimately lives, so it is the one file that has to
        # be checked separately. Publishing an artifact with this machine's
        # address in it would hand every downloader a guest that dials THIS
        # host and reports full health: R2, multiplied by the user base.
        if release:
            found = payload_host_literals(prefs_text.encode("mac_roman"))
            if found:
                return (False, "refusing to publish a release kit whose prefs "
                               "carry an address: " + ", ".join(found)
                               + ". A released kit must ship IP= empty; the "
                                 "downloader sets it with --seed-guest-prefs "
                                 "or in AppleBridgeConfig.", [])
    ...
    msg = (f"kit image with {len(placed)} items: {image_path}. "
           ...
           + ("Its prefs carry NO address, because a published kit must not "
              "carry the building machine's: stamp it on the machine that will "
              f"run the bridge with `--seed-guest-prefs {image_path}`, or set "
              "IP= in AppleBridgeConfig on the guest afterwards."
